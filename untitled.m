...
assignin('base','sn',sn);
x=randi([0,400],1,sn); % x coordinate ofCH
y=randi([0,400],1,sn); % y coordinate of CH
assignin('base','x',x);
assignin('base','y',y);
save ('myvars','x','y');
plot(x,y,'*b')
...
if wsn_e(rowi, coli) > e_thr
            ...
            e_rounds{1, round} = wsn_e; 
            total_energy(round) = sum(sum(e_rounds{1, round}, 2), 1);
end
        figure (2);
...
if wsn_e1(rowi1, coli1) > e_thr
            ...
            e_rounds1{1, round1} = wsn_e1;
            total_energy1(round1) = sum(sum(e_rounds1{1, round1}, 2), 1); 
end
        figure (2);
